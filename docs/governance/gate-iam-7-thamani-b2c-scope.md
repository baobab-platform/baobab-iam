...
- **Both workload clients share one secret** (`BOOTSTRAP_WORKLOAD_CLIENT_SECRET`, per `scripts/bootstrap.sh`, for CI/local reproducibility) — so "wrong client_id/secret pairing is rejected" is not a meaningful test in this environment; what actually proves isolation regardless of shared secrets is that Keycloak binds `azp`/`sub` to whichever `client_id` authenticated, never to which secret string was used.

`tests/integration/run.sh` §20-21 add four checks, all verified passing against a real instance (including a sanity check that deliberately broke `thamani-web`'s redirect URI registration and confirmed the test would have caught it — 302 broken vs. 400 correct):
...
3. `thamani-web` requesting `zuribeans-web`'s registered redirect URI is rejected by Keycloak itself (400).
4. `thamani-web` requesting its own redirect URI is accepted (302).
